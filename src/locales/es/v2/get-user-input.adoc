...
Vamos a ver cómo pedir sugerencias de los usuarios cuando se oprima el botón de "ejecutar". Esto es posible usando la función `readInput()`. Esta función requiere como parámetro una cadena (<em>string</em>), tal como ("¿qué tempo te gustaría tener en tu música?"). Cuando el usuario ejecute el código, verá una nueva ventana con la cadena de caracteres (o <em>string</em> en inglés) y se le pedirá que escriba una respuesta. La función `readInput()` devuelve lo que el usuario haya escrito allí.
...
Ahora daremos otro ejemplo de interacción con el usuario en el cual combinaremos la lógica boolena y la respuesta (o <em>input</em>) presentada por el usuario.
