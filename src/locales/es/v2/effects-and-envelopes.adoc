...
* *Las cadenas*. Una cadena de tiempos, como `"0000---0000---"`, se usa como argumento en la función `makeBeat()`.
...
# Esto es un _fade in_. setEffect(1, VOLUME, GAIN, -60, 1, 0, 3)
...
setEffect(1, VOLUME, GAIN, 0, puntoD, -10, puntoE)  # Empieza el desvanecimiento del volumen (_fade out_). setEffect(1, VOLUME, GAIN, -10, puntoE, -60, puntoF) # Fin del _fade out_
...
.PRÁCTICA
****
Crea una nueva canción. Crea una nueva canción. Puedes usar el efecto que quieras.
