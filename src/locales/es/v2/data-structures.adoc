...
** ¿Copié y pegué llamadas a función múltiples veces seguidas? If so, simplify with a loop!
** ¿Estoy volviendo a usar algunos bloques de código? Si la respuesta es sí, ¡ponlos en una función!
** ¿Hay constantes de sonido o expresiones matemáticas que uso repetidamente en mi código? If so, assign them to variables! Una lista también podría ser útil.
...
** ¿Son descriptivos los nombres de mis variables y funciones? If not, rename them.
