...
[[effectsinearsketch]]
=== Cómo usar efectos en EarSketch

Los *efectos* nos permiten cambiar las características de los sonidos. De modo similar a como añadir un filtro altera una foto, añadir un efecto de audio cambia el sonido. Por ejemplo, puedes cambiar el volumen, añadir un poco de eco o reverberación...
...
. *valorDelEfecto*: El valor del parámetro: un número en un rango específico. El rango depende del parámetro del efecto.
...
. Descomenta la primera línea `setEffect()` y escucha la diferencia ("descomentar" significa borrar la sintaxis que se usa para los comentarios, "#" o "//", del bloque de código)
. Descomenta la segunda línea `setEffect()` y escucha la diferencia
Toma en cuenta que la duración del efecto de retardo (_delay_), 500ms, es la misma duración que una pulsación. Ya que el tempo es 120 pulsaciones por minuto, hay una pulsación cada 60/120 = 1/2 de segundo. 1 segundo son 1000ms, entonces una 1/2 de segundo son 1000/2 = 500ms. Puedes tratar de modificar la duración del efecto de retardo y observar si tu música suena mejor o no.
****
...
* *Números*. *Enteros* (o "int") son números enteros, como 0, 5, o -26. Los números de *coma flotante* (_floating point_ en inglés) son números racionales como 0,125 o -21,0. Por ejemplo, los argumentos de número de pista, compás inicial y compás final de la función `fitMedia()` son todos números.
* *Cadenas* son también conocidas como cadenas de caracteres en español o _strings_ en inglés. Una cadena de tiempos, como `"0000---0000---"`, se usa como argumento en la función `makeBeat()`.
...
.PRÁCTICA
****
Ve a este capítulo para una lista completa de efectos: <</en/v1/every-effect-explained-in-detail#,effects>>.
...
. effectStartLocation (compasInicialDelEfecto)
. effectEndValue (valorFinalDelEfecto)
. effectEndLocation (compasFinalDelEfecto)

Los últimos 3 de los 7 parámetros son *parámetros opcionales*. Si no se especifican, como fue el caso cuando sólo asignamos 4 parámetros a seEffect(), el efecto se aplicará a la pista entera.
...
.PRÁCTICA
****
Crea una nueva canción. Usa un ciclo _for_ para añadir un envelope a todas las pistas (por ejemplo: un _fade in_ y _fade out_ en todas tus pistas), o repetir un efecto en la misma pista. Puedes usar el efecto que quieras.
...
** *numeroDePista:* La pista musical a la cual se añade el efecto.
** *nombreDelEfecto:* El efecto específico que se usa.
** *parametroParaElEfecto:* La configuración que se usa para el efecto.
** *valorDelEfecto:* El valor del parámetro: un número en un rango especifico.*
* Las *funciones* contienen instrucciones que la computadora ejecuta. Se envían datos a las funciones a través de *argumentos*, que afectan cómo se ejecuta la función. Un ejemplo de la sintaxis de una *llamada* a función con 4 argumentos es `makeBeat(bombo, 2, compas, ritmoDeBombo)`.
...
<script>
question1 = new Question(1, "¿Qué te permite hacer un efecto en EarSketch?", "Cambiar las características de sonido dentro del proyecto", "Añadir un sonido a una pista musical", "Crear un ritmo de tambor", "Cambiar el tempo de una canción");
question2 = new Question(2, "¿Cuál de éstos NO es un argumento setEffect()?", "Nombre del clip", "Nombre del efecto", "Valor del efecto", "Número de pista musical");
question3 = new Question(3, "¿Cómo asignarías un tiempo de retardo (_delay_) de 50 milisegundos a un efecto de retardo en la pista musical 3?", "setEffect(3, DELAY, DELAY_TIME, 50.0)", "setEffect(DELAY, 3, DELAY_TIME, 50.0)", "fitMedia(DELAY, 3, DELAY_TIME, 50.0)", "setEffect(50, DELAY_FEEDBACK, 1)");
question4 = new Question(4, "¿Cuál de las siguientes opciones no es un parámetro que se usa con los envelopes setEffect()?", "Duración del clip", "Valor de comienzo", "Número de pista musical", "Efecto");
question5 = new Question(5, "¿Qué haría la siguiente función setEffect()? setEffect(1, DISTORTION, DISTO_GAIN, 0, 1, 50, 10)", "Aumentar la intensidad de la distorsión de la pista 1 durante 10 compases.", "Reducir la intensidad de la distorsión de la pista 1 durante 50 compases.", "Aumentar el volumen de la pista 1 durante 10 compases.", "Reducir el volumen de la pista 1 durante 50 compases.");
</script>
++++
