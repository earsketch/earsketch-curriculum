...
In programming we can create *abstractions*:  bundling of ideas to form a single concept. Just like in musicjust as we group musical ideas into sections. Functions are one kind of abstraction used in computer science. They pack multiple statements into one tool so they can be easily referred to. Abstractions can make the form of a program clearer.
...
. Then select a simple structure. For each section, try to think of some
. And finally, start coding! Start by selecting sounds and putting them in `fitMedia()` functions.
