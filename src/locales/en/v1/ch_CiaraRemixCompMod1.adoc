...
In the basic script provided, you will notice code such as `init()`, `setTempo()`, `fitMedia()`, and `finish()`. These are called functions. Functions tell the computer what to do based on information, known as arguments. The parentheses after a function provide space for arguments. The first function `init()` informs the computer of the start of the code.
