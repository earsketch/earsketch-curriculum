...
* *Loop Body:* The body of a loop contains statements that will execute repeatedly. It includes everything that is indented (using the tab key) directly after the colon., In our example, the loop body was our two makeBeat functions.
...
. *Print debugging can also be used to locate an error:* Insert print statements throughout your code `fitmedia(RD_EDM_DRUM BEATPART_1, 10, 1, 4)` , to obtain the value of variables and check your logic.
