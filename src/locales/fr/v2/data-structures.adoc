...
[role="curriculum-python"]
Nous avons vu que  `list[index]` va invoquer un élément de la liste. Ceci peut également être utilisé pour modifier un élément de la liste. Par exemple : `myEnsemble[0] = RD_ROCK_POPRHYTHM_MAINDRUMS_11` dans l'exemple précédent va modifier le premier élément de la liste en `RD_ROCK_POPRHYTHM_MAINDRUMS_11`. Ce processus est similaire à la mise à jour d'une variable, il utilise l'opérateur d'affectation `=`.

[role="curriculum-javascript"]
Nous avons vu que `array[index]` va invoquer un élément du tableau. Ceci peut également être utilisé pour modifier un élément du tableau. Par exemple : `myEnsemble[0] = RD_ROCK_POPRHYTHM_MAINDRUMS_11` dans l'exemple précédent va modifier le premier élément du tableau en `RD_ROCK_POPRHYTHM_MAINDRUMS_11`. Ce processus est similaire à la mise à jour d'une variable, il utilise l'opérateur d'affectation `=`.
