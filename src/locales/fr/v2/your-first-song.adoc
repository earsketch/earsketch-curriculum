...
. Jouez votre chanson selon trois tempos différents pour un.e ami.e. Demandez-lui ensuite de classer les morceaux du tempo le plus lent au plus rapide. Then ask them to rank from slowest to fastest tempo.
****
...
Il y a deux droits d'auteur liés à une chanson : les droits de la chanson (par l'auteur ou le compositeur) et les droits d'enregistrement sonore (souvent par la maison de disques). Royalties from public performances go to the songwriter and most royalties from record sales go to the record label.
