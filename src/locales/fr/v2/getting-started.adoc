...
Pour ajouter un clip de son au DAW, nous commençons par entrer la fonction `fitMedia()`. Entre les parenthèses, nous aurons quatre arguments, séparés par des virgules :
...
. *Le numéro de piste* : chaque piste correspond à un type d'instruments (voix, guitare soliste, guitare d'accompagnement, basse, batterie, etc.). Dans le DAW, une piste est une ligne. Vous pouvez commencer à la piste numéro 1 pour votre premier son.
...
Voici un exemple de code utilisant la fonction `fitMedia()` :
...
. *Sensibilité à la casse :* La plupart des mots utilisés en programmation sont sensibles à la casse (l'ordinateur reconnaît la différence entre les lettres majuscules et minuscules). Par exemple, vous devez écrire `fitMedia()` et non `FitMedia()` ou `fitmedia()`. La plupart des éléments d'un script suivent une règle appelée la *notation "Camel Case"* : c'est-à-dire que le premier mot est en minuscule et que la première lettre des mots suivants sera en majuscule, comme dans `exempleNomFonction()`.
. *Parenthesès :* Oublier d'ouvrir ou de fermer une parenthèse peut causer une <</en/v1/every-error-explained-in-detail#syntaxerror, erreur de syntaxe>>.
. *Paramétrage du script :* EarSketch ajoute automatiquement des code de paramétrage à un nouveau script, mais vous pourriez accidentellement supprimer `from earsketch import *`. Si c'est le cas, réécrivez `from earsketch import *` au début de votre code.
...
. Il manque une virgule entre le troisième et le quatrième argument de la première fonction `fitMedia()`.
. Il manque un « M » (majuscule) à la deuxième fonction `fitMedia()`.
. Dans la seconde fonction `fitMedia()`, l'ordre des arguments est incorrect : on devrait d'abord retrouver le nom du clip de son, ensuite le numéro de piste.

////
END OF OPTIONAL
////

Regardez le chapitre <</en/v1/every-error-explained-in-detail#, Toutes les erreurs expliquées en détails>> pour avoir une liste plus complète d'erreurs et de techniques pour les résoudre.
...
* Les clips de sons peuvent être trouvés dans la bibliothèque de sons. Il est possible de les utiliser en les collant dans le code : cliquez sur l'icone bleue à droite du nom du son.
