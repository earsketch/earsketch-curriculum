...
== Utiliser des fonctions personnalisées pour la structure de votre chanson
:nofooter:

Maintenant que vous disposez de plusieurs d'outils pour créer votre chanson, ajouter des effets et mixer vos morceaux, nous allons examiner la structure d'une chanson. La structure représente son organisation dans le temps et consiste à créer des sections et des transitions pour assurer le flow de votre musique. Pour ce faire, nous allons utiliser des fonctions personnalisées, c'est à dire des fonctions que vous allez créer vous-même !
...
def sectionA():
	...
	fitMedia(RD_UK_HOUSE__EVOLVEPAD_2, 3, 1, 5) # bassline
	setEffect(3,VOLUME,GAIN,-20,1,0,5) # increasing volume of bass line
	# This code is indented. Lorsque vous écrivez le reste de votre script, assurez-vous d'arrêter l'indentation.
----
...
. ajoutez les paramètres `startMeasure` et `endMeasure` séparés par une virgule entre les parenthèses de la fonction sectionA() dans sa définition. (`sectionA(startMeasure,endMeasure)`).
. Dans le corps de la fonction, remplacez les mesures de début (1) et de fin (5) par `startMeasure` et `endMeasure` respectivement.
. Lorsque vous appelez votre fonction, ajoutez les paramètres `1` et `5` entre les parenthèses. Exécutez le code pour vous assurer qu'il n'y a aucune erreur.
. Ajoutez un deuxième appel de fonction, mais cette fois-ci, avec les paramètres `9` et `13`. Exécutez le code pour vous assurer qu'il n'y a aucune erreur.
. Définissez une fonction pour la partie B, en suivant le même processus, et appelez la partie B des mesures 5 à 9 et des mesures 13 à 17.
****
...
. Sélectionnez ensuite une structure simple. Essayez de trouver quelques idées pour chaque partie.
