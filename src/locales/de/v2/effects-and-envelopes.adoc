...
.ÜBUNG
****
Erstelle einen neuen Song. Benutze einen for-loop, um einen Umschlag zu allen Tracks hinzuzufügen (z.B. Ein- und Ausblenden für alle Tracks möglich), oder um einen Effekt auf der gleiche Spur zu wiederholen. Sie können jeden gewünschten Effekt verwenden.
...
* Das *Volume* bezieht sich auf die Lautstärke. *Verzögerung* erzeugt ein Echo. *Reverb* fühlt sich an, als würde der Ton in einem großen Raum gespielt. *Panning* platziert deine Musik auf der linken oder rechten Seite.
...
* *Umschläge* definieren, wie sich ein Effektparameter im Laufe der Zeit ändert. Sie werden mit Wertzeitpaaren wie _(Wert, Zeit, Wert, Zeit)_beschrieben.
* Für einen Umschlag lauten die 7-Parameter-Argumente `setEffect()`: `setEffect(track, type, parameter, startValue, start, endValue, end)`.
...
[question]
--
Welcher der folgenden Parameter ist kein Parameter mit `setEffect()` Umschlägen?
