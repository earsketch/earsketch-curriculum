...
Die Songstruktur ist die Organisation des Songs im Laufe der Zeit. Das Ziel ist es, sinnvolle Abschnitte und Übergänge für den Fluss Ihrer Musik zu erstellen. In diesem Kapitel wirst du deine eigene Funktionen, benutzerdefinierte Funktionen, erstellen, um deine Songs zu strukturieren!
...
Nun wollen wir Abschnitt A von den Takten neun bis dreizehn hinzufügen. Wenn wir jedoch `sectionA()` aufrufen, werden die Töne von Takt eins bis fünf platziert. Um dieses Problem zu beheben, erstellen wir *Parameter* für unsere Funktion.
