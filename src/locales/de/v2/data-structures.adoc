...
Jetzt stehen Ihnen zahlreiche Tools zur Verfügung, um Lieder mit EarSketch zu erstellen. Die Skripte, die du schreibst, können sehr vielfältig und lang werden. Dieses Kapitel hilft dir, deinen Code weiter zu organisieren, insbesondere mit Hilfe einer speziellen Abstraktion namens Listen, und einer neuen Funktionalität von `makeBeat()`.
...
[role="curriculum-python"]
Wenn du sehr lange Listen von Klangvariablen hast, ist es möglich, diese mit *listen* zu organisieren. Eine *Liste*, in manchen Programmiersprachen auch ein Array genannt, ist eine Sammlung von Werten, oder *Elementen*, zwischen Klammern (`[]`) und durch Kommata getrennt. Hier ein Beispiel:

[role="curriculum-javascript"]
Wenn du sehr lange Listen von Klangvariablen hast, ist es möglich, diese mit *arrays* zu organisieren. Ein *array*, auch in einigen Programmiersprachen eine Liste genannt, ist eine Sammlung von Werten, oder *Elementen*, zwischen Klammern (`[]`) und durch Kommata getrennt. Hier ein Beispiel:
...
Mit drei `fitMedia()` werden dann drei Sounds auf die Titel `1`, `2` und `3` von Takt `1` bis `5` platziert.
****
...
.ÜBUNG
****
Wir werden eine additive Einführung in EarSketch erstellen, in der Instrumente (Tracks) im Laufe der Zeit einzeln der Musik hinzugefügt werden. Zum Beispiel beginnt der erste Track bei Maß `1`, dann beginnt der zweite Track bei Maß `2`, und so weiter. Dies wird häufig für die Einleitung eines Songs verwendet.
...
. `stringC`, die Verkettung von `stringA` und `stringB`, erneut erstellen und ausgeben.
...
[role="curriculum-python"]
Schließlich kann `makeBeat()` mehrere Sounds auf einmal bewältigen. So kannst du alle Percussive Sounds mit nur einer `makeBeat()` Linie auf einen Track platzieren. Anstatt deine Sounddatei als erstes Argument zu übergeben, übergibst du eine Liste von Sounds. In deinem Beat String kannst du statt nur mit `0` einen Ton abspielen, Zahlen von `0` bis `9` verwenden. Diese Zahlen zeigen einen Index in deiner Liste der Sounds an. Schaue dir das Beispiel „vorher und nachher“ an, mit dem „Boom Bap Boom Bap“-Rhythmus.

[role="curriculum-javascript"]
Schließlich kann `makeBeat()` mehrere Sounds auf einmal bewältigen. So kannst du alle Percussive Sounds mit nur einer `makeBeat()` Linie auf einen Track platzieren. Anstatt deine Sounddatei als erstes Argument zu übergeben, übergibst du einen Array von Sounds. In deinem Beat String kannst du statt nur mit `0` einen Ton abspielen, Zahlen von `0` bis `9` verwenden. Diese Zahlen zeigen einen Index in Ihrem Array von Sounds. Schaue dir das Beispiel „vorher und nachher“ an, mit dem „Boom Bap Boom Bap“-Rhythmus.
...
Herzlichen Glückwunsch, jetzt hast du musikalische und programmiertechnische Fähigkeiten erworben, die es dir ermöglichen, mit EarSketch kreativ zu sein! Eine letzte Sache, die wir sicherstellen wollen, ist, dass Ihr Code so sauber wie möglich ist. Hier sind einige Elemente, die du überprüfen kannst:
...
* Du kannst eine bedingte Anweisung verwenden, um deinen Song zu mischen (passe die Lautstärke der einzelnen Spuren an).
...
* `makeBeat()` kann Rhythmen aus mehreren Klängen gleichzeitig konstruieren, indem er in einem Beat-String übergeht, der auf verschiedene Listenindizes verweist. `makeBeat()` kann auf Sounds in Indizes 0 bis 9 zugreifen, vorausgesetzt, sie werden in der gleichen Liste gespeichert.
