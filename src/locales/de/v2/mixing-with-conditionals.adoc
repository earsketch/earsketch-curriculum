...
* Verwende einen for-loop mit dem Zähler `track`, um für jede Spur zu prüfen, ob die Lautheit größer als 0,01 ist. Wenn ja, wird `True` ausgegeben, wenn nicht, wird `False` ausgegeben.
...
* Wähle deinen "Haupt"-Track. Es kann deine Melodie sein, oder eine Spur den du hervorheben möchtest.
...
Unten findest du ein Beispiel für automatisches Mischen. Man kann sagen, dass es automatisiert ist, denn wenn du einen oder mehrere Töne änderst, musst du nicht selbst die Lautstärke überprüfen und entsprechend anpassen, da dies bereits im Code enthalten ist.
