...
Schaue dir dieses Video an, um zu sehen, wie du einen Audioclip zu deinem Song hinzufügst:
...
Wenn du alle deine Parameter hinzugefügt hast, drücke _run_. Du wirst deine Sounds in der DAW visualisiert sehen. Drücke _play_ um deinen Song zu hören.
...
Zu den Mitwirkenden gehören prominente Musiker
...
Du kannst den Soundbrowser im *Content Manager* (links) erkunden, indem du auf „Sounds“ klickst. Versuche es mit verschiedenen Soundkonstanten innerhalb von `fitMedia()`.
...
. *Skripteinrichtung:* EarSketch fügt einem neuen Skript automatisch etwas Code hinzu, aber es kann sein, dass Sie versehentlich `von earsketch import *` löschen.
...
. Der ersten `fitMedia()` fehlt ein `e`
...
Unter <</en/v1/every-error-explained-in-detail#,Every Error Explained in Detail>> findest du eine Beschreibung der verschiedenen Fehlertypen und was du tun kannst, um sie zu vermeiden.
...
* *Sounds* kannst du im Sound-Browser finden. Du kannst Klänge zu deinem Code hinzufügen, indem du die *Soundkonstante* in einer Funktion wie `fitMedia()` benutzt.
