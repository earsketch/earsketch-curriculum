...
.. *Print Debugging* kann auch verwendet werden, um einen Fehler zu lokalisieren. Lese den Problembereich deines Codes und versuche der Logik zu folgen. Insert `print()` calls where you are unsure of the logic, getting the value of variables and checking program state. This helps you check your understanding of the program against what is actually happening.
...
.. Wenn die Konsole eine Zeilennummer angegeben hat, werfe einen Blick auf diese Zeile und die vorherige Zeile in dinem Code.
.. Verwenden die "comment out"-Methode. Du kannst einen Fehler eingrenzen, indem du einen Codeblock mit Kommentarsyntax versiehst oder ihn *auskommentierst* und den Code ausführst. Wenn es keinen Fehler gibt, ist der Fehler irgendwo im kommentierten Block.
.. *Print Debugging* kann auch verwendet werden, um einen Fehler zu lokalisieren. Lese den Problembereich deines Codes und versuche der Logik zu folgen. Insert `println()` calls where you are unsure of the logic, getting the value of variables and checking program state. This helps you check your understanding of the program against what is actually happening.
. *Squash the bug:* Check for errors and edit the offending code, then run it to verify its correctness.
. *Ask for help:* If you find you have spent too much time on a bug, then ask someone for help! A fresh pair of eyes can do wonders for spotting mistakes.
...
These are basic ideas that create the structure of your song's texture. However, you can have some parts of your song that only contain one or two of the four. You can also add a lot more tracks: you can create a second melody, add drones (very long notes in the background), recorded sounds, whooshes, etc. Die Möglichkeiten sind endlos! Probiere Ideen aus und behalte die, die dir am besten gefallen!
