...
So wie Musiker in einer *Schleif* immer wieder ein Muster spielen können, können Programmierer den Computer immer wieder auffordern, Aufgaben zu erledigen. in einem *loop*! Das ist effizienter: Anstatt eine Anweisung mehrmals zu schreiben, schreibst du einen Loop mit den Anweisungen, und der Computer weiß, dass er sie wiederholen soll.
...
* *Loop Bedingung*: Diese prüft, ob die Schleife erneut ausgeführt werden soll. Wenn die Anweisung wahr ist, wird der Schleifenkörper erneut ausgeführt. Wenn der Zähler zu hoch wird, ist die Aussage false, und wir verlassen den Loop. Der Computer führt dann den Code nach dem Loop weiter aus.
...
und es endet dort, da die Messung kleiner als 5 sein muss, also 4 ist Ihr Limit.
...
Wir können einen `for` loop verwenden, um genau die gleiche Musik mit weniger Code zu erstellen. Unser Zähler hier ist "measure". Beachten Sie, dass der Loop Body zwei Codezeilen enthält, die beide den Zähler `measure` verwenden.
...
Diese Animation zeigt, wie sich der Kontrollfluss in einem `for` loop bewegt, und wie sich der Wert des loop counters bei jeder *iteration*, oder Wiederholung des Schleifenkörpers ändert:
...
Programmieren ist nicht nur das Schreiben von Code. Es ist auch das Debuggen und Pflege davon. Debuggen bedeutet, Fehler zu finden und zu lösen. Bugs sind ein anderer Begriff für Fehler in Ihrem Code. Versuche diesen Schritten zu folgen, wenn du einen Fehler hast:
...
. *Squash den Bug:* Prüfe auf Fehler, bearbeite den Code und führe ihn dann aus, um seine Richtigkeit zu überprüfen.
...
Jetzt, wo du viele Werkzeuge zum Erstellen deiner Musik, wie `fitMedia()`, `makeBeat()` und `for`-Schleifen hast, werden wir uns musikalische Ideen ansehen.

Beginnen wir mit der *Tonart* deines Songs:

* *Pitch* ist, wie hoch oder niedrig ein Ton klingt. Wir ordnen relative musikalische Töne auf einer *Skala* oder einer Reihe von Musiknoten, basierend darauf, wie wir die Frequenz des Tons hören.
...
* Anfängern empfehlen wir, nur eine Tonart für ihr Lied zu verwenden. Das Auswählen von Tönen aus verschiedenen Tonarten kann sich... ziemlich schief anhören! In der Regel sind alle Sounds innerhalb desselben Ordners in der EarSketch-Soundbibliothek in derselben Tonart.
...
Lass uns nun über die verschiedenen Arten von Tracks, die du haben kannst, sprechen. Du erinnerst dich vielleicht daran, dass du für jede Art von Instrument einen Track in deiner DAW verwenden kannst. In einem Pop Song findest du die folgenden Basistracks:
...
* Dann gibt es *Percussion*. Wenn du `makeBeat()` verwendest, kann das mehrere Spuren benötigen. Du kannst zum Beispiel eine Spur für deine Kick, eine für deine Snare und eine für deine Hi-Hat haben.

Dies sind grundlegende Ideen, die die Struktur deines Songs bilden. Du kannst jedoch einige Teile deines Songs haben, die nur eine oder zwei der vier enthalten. Du kannst auch viele weitere Spuren hinzufügen: Du kannst eine zweite Melodie erstellen oder Drones (sehr lange Noten im Hintergrund), aufgenommene Geräusche, Whooshes usw. hinzufügen. Die Möglichkeiten sind endlos! Probiere Ideen aus und behalte die, die dir am besten gefallen!
...
Der Kontrast bezieht sich auf Unterschiede in aufeinander folgenden Abschnitten der Musik, die ein wichtiges Gleichgewicht zur Wiederholung darstellen. Der Kontrast wird benutzt, um dem Zuhörer neue Elemente zu zeigen. Musiker setzen Kontraste durch rhythmische Veränderungen, neue melodische Linien und Harmonien oder Variationen bei den verwendeten Instrumenten oder Klängen.
...
* Mindestens sechzehn Takten
* Mindestens einem hochgeladener Sound
...
* Der *Pitch (Tonhöhe)* eines Klangs bestimmt, wie hoch oder niedrig er auf einer relativen Skala klingt.
...
* Du kannst 3 Grundspuren für das Rückgrat deiner Songs verwenden: eine Melodie in höheren Tonlagen, einen Bass in tieferen Tonlagen und ein Schlagzeug.
...
[question]
--
____ ist eine Tonqualität, die bestimmt, wie hoch oder niedrig es klingt.
