...
[role="curriculum-javascript"]
Ushbu skript uchun kod muharriri bizga noto'g'ri satrni belgilaydi. Notice that we forgot to open our _if_ statement body with a curly brace. On the same line, we also gave an invalid condition; we should be using the strict equality operator `===` instead of the assignment operator `=`.
